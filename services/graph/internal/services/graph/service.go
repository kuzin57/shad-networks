...
func (s *Service) FindPath(ctx context.Context, graphID string, source, target int) (entities.Graph, entities.Path, []byte, error) {
	exists, err := s.cache.CheckGraphExists(ctx, graphID)
	switch {
	case err != nil:
		return entities.Graph{}, nil, nil, fmt.Errorf("check graph exists: %w", err)
	case !exists:
		if err := s.repo.ProjectIfNotExists(ctx, graphID); err != nil {
			return entities.Graph{}, nil, nil, fmt.Errorf("project graph: %w", err)
		}
	}

	graph, err := s.repo.GetGraph(ctx, graphID)
	if err != nil {
		return entities.Graph{}, nil, nil, fmt.Errorf("get graph: %w", err)
	}

	path, err := s.repo.FindPath(ctx, graphID, source, target)
	if err != nil {
		return entities.Graph{}, nil, nil, fmt.Errorf("find path: %w", err)
	}
	...
	if err != nil {
		...
		return entities.Graph{}, nil, nil, fmt.Errorf("visualize image: %w", err)
	}

	return graph, path, b64Image, nil
}
